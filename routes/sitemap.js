...
module.exports = function sitemap(req, res) {
  ...
  const areas  = db.prepare("SELECT DISTINCT area FROM properties WHERE active=1 AND area != '' ORDER BY area").all().map(r => r.area);
  ...
  <url>
    ...
    // local-landing pages per area (e.g. property in Udupi / Mangaluru)
    ...areas.map(a => `
  <url>
    <loc>${base}/properties?area=${encodeURIComponent(a)}</loc>
